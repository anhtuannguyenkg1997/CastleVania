...
public:
	//Constructor
	//Thời gian mâc định của 1 Frame là 0,1 giây
	...
	//Biến isStop để kiểm tra việc render có bị dừng lại hay không? ví dụ như dùng stopwatch
	void Render(int accordingCam, bool isStop, int nx, float x, float y, int alpha = 255);

	// hàm dùng riêng để render MorningStar
	void RenderMorningStar(int simonFrameID, int nx, float x, float y, int alpha = 255);
};
