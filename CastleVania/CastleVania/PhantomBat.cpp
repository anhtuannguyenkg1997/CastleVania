...
void PhantomBat::Update(DWORD dt, vector<LPGAMEOBJECT>& listObjects, Simon* simon)
{
	...
	if (coEvents.size() == 0)
	{
		//Chưa bị tiêu diệt hoặc không dùng stopwatch
		if (state != ENEMY_DESTROYED && !SubWeaponManager::GetInstance()->isUseStopWatch) {
...
//Lấy từ danh sách listObject. tìm ra các vật có thể va chạm với PhantomBat gán vào coObjects
