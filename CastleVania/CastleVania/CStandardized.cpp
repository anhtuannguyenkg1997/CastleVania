...
void StandardizedText(string& score, string& remainTime, string& idScene, string& subWeaponCounter, string& live) {

	while (score.length() < HUD_TEXT_SCORE_LEN) score = "0" + score;

	while (remainTime.length() < HUD_TEXT_TIME_LEN) remainTime = "0" + remainTime;

	while (idScene.length() < HUD_TEXT_IDSCENE_LEN) idScene = "0" + idScene;
