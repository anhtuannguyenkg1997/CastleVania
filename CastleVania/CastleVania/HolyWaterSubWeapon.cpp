...
void HolyWaterSubWeapon::GetCollisionObject(vector<LPGAMEOBJECT> listObjects, vector<LPGAMEOBJECT>& coObjects) {
	...
	{
		//Trạng thái lửa cháy mới cho va chạm với Object khác
		if (state == W_HOLYWATER_FIRE_STATE) {
			if (dynamic_cast<Candle*>(obj) || dynamic_cast<Torch*>(obj))
				coObjects.push_back(obj);
			else if ((dynamic_cast<Enemy*>(obj)) && obj->GetState() == ENEMY_WORKING && !obj->getIsImmortal())
				coObjects.push_back(obj);
		}
		

		//Nếu không chỉ cho va chạm với Brick
