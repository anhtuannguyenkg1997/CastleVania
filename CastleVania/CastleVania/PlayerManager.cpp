...
//Tính điểm khi ăn MagicCrystal
void PlayerManager::CalculateScore() {

	//Lấy thời gian còn lại để tính điểm
	int TimeRemain = SceneManager::GetInstance()->GetCurrentScene()->GetRemainTime();

	//Khởi tạo Temp time để tính từng timeRemain và SmallHeart còn lại
	if (timeCalcScore == -1) timeCalcScore = GetTickCount();


	//Còn timeremain và hết 50ms . Tức là 50ms chạy vào khói if này 1 lần
	if (TimeRemain > 0  && GetTickCount() - timeCalcScore > HUD_TIME_SUB_PER_TIMESEC) {
		
		//Mỗi lần trừ đi 1s => điểm
		TimeRemain -= 1000;
		...
		//Ghi lại thời gian để tiếp tục tính điểm
		timeCalcScore = GetTickCount();

	}
	//Đã tính xong timeRemain
	//Còn smallHeart và hết 300ms . Tức là 300ms chạy vào khói if này 1 lần
	else if(TimeRemain <= 0 && GetTickCount() - timeCalcScore > HUD_TIME_SUB_PER_SMALLHEART && subweaponCounter > 0){

		//Mất 1 smallheart => điểm
		subweaponCounter--;
