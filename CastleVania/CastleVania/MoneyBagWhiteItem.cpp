...
MoneyBagWhiteItem::MoneyBagWhiteItem(float x, float y) : Item(x, y) {

	//Lấy Animation Của Item
	CAnimationSets* animation_sets = CAnimationSets::GetInstance();
	animation_set  = animation_sets->Get(ITEM_MONEYBAG_WHITE);
}


void MoneyBagWhiteItem::Render() {
	Item::Render();
}



void MoneyBagWhiteItem::Update(DWORD dt, vector<LPGAMEOBJECT> &listObjects, Simon* simon) {
	...
	if (isEffectToSimon) {
		...
		temp += ITEM_MONEYBAG_WHITE_SCORE;
		playerManager->SetScore(temp);

		//Hiển thị số điểm lên màn hình game
		playerManager->idScoreShow = ITEM_MONEYBAG_WHITE_SCORE;
...
void MoneyBagWhiteItem::GetBoundingBox(float& left, float& top, float& right, float& bottom) {
