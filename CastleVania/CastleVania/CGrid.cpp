...
vector<CObjectCell*> CGrid::GetObjectCell()
{
	...
	for (int i = 0; i < rows; i++)
	{
		for (int j = fromCol; j < toCol; j++)
		{
			...
			for (int k = 0; k < cell->GetObjectList().size(); k++) {

				//Lấy từng GameObject trong Cell đang xét 
				LPGAMEOBJECT obj = cell->GetObjectList().at(k)->GetObj();

				//Những Enemy cần hồi sinh sẽ được thêm vào list Revival
				//Lúc này Enemy vẫn nằm trong vùng camera và chờ khi camera đi khỏi vị trí của Enemy thì nó sẽ Enable trở lại
				if (obj->isNeedRevival) {
					
					//Thêm nó vào listRevival
					EnemyRevival::GetInstance()->AddToListRevival(cell->GetObjectList().at(k));
					
					//tắt cờ cho phép hồi sinh 
					cell->GetObjectList().at(k)->GetObj()->isNeedRevival = false;
					continue;
				}
					

				//Đưa các brick vào list 
				//Nếu đang đấu boss thì mới đưa BOSS_BRICK vào list để tạo vùng bao quanh màn đấu 
				if (dynamic_cast<Brick*>(obj) && obj->GetType() == BOSS_BRICK &&  playerManager->GetIsFightingBoss()) {
					listObjectCells.push_back(cell->GetObjectList().at(k));
					continue;
				}
				...
				//Nếu object còn nằm trong camera và đang hoạt động thì đưa vào list
...
void CGrid::Update(vector<CObjectCell*> &listObjectCells) {
	...
	for (int i = 0; i < listObjectCells.size(); i++)
	{
		...
		if (!camera->checkObjectInCamera(listObjectCells[i]->GetObj())) {
			...
			if (dynamic_cast<SubWeapon*>(listObjectCells[i]->GetObj()) || dynamic_cast<VampireBat*>(listObjectCells[i]->GetObj())) {
...
void CGrid::ShowSizeCells() {
	...
	//In kích thước của mảng objectCell 
	for (int i = 0; i < rows; i++) {
		for (int j = fromCol; j <= toCol; j++) {
