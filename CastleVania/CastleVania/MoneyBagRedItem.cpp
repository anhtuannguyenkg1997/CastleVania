...
MoneyBagRedItem::MoneyBagRedItem(float x, float y) : Item(x, y) {

	//Lấy Animation Của Item
	CAnimationSets* animation_sets = CAnimationSets::GetInstance();
	animation_set = animation_sets->Get(ITEM_MONEYBAG_RED);

}


void MoneyBagRedItem::Render() {
	Item::Render();
}



void MoneyBagRedItem::Update(DWORD dt, vector<LPGAMEOBJECT> &listObjects, Simon* simon) {
	...
	if (isEffectToSimon) {
		...
		temp += ITEM_MONEYBAG_RED_SCORE;
		playerManager->SetScore(temp);

		//Hiển thị số điểm lên màn hình game
		playerManager->idScoreShow = ITEM_MONEYBAG_RED_SCORE;
...
void MoneyBagRedItem::GetBoundingBox(float& left, float& top, float& right, float& bottom) {
