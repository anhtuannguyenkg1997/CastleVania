...
MorningStar::MorningStar() : CGameObject()
{
	...
	//Lấy state của MS từ playerManager
	//Mỗi lần load lại scene thì phải tạo lại MS
	//Mà khi lưu state của MS trong playerManager thì không phải bận tâm khi chuyển scene thì state của MS bị thay đổi
...
void MorningStar::Update(DWORD dt, Simon* simon, vector<LPGAMEOBJECT>& listObjects)
{
	...
	if ((simon->GetState() == WHOPPING_SITTING || simon->GetState() == WHOPPING_STANDING || simon->GetState() == WALKING_DOWN_STAIR_WHOPPING || simon->GetState() == WALKING_UP_STAIR_WHOPPING) 
		&&simon->animation_set->at(simon->GetState())->IsRenderingLastFrame()
		&& !subweaponManager->isHitBySubweapon
		)
	{
		...
		for (UINT i = 0; i < coObjects.size(); i++)
		{
			...
			//Va chạm MorningStar và Torch, Candle
			if (dynamic_cast<Torch*>(obj)|| dynamic_cast<Candle*>(obj))
			{
				...
				if (CheckCollision(left, top, right, bottom) == true)
				{
					...
					obj->SetState(STATIC_OBJECT_DESTROYED);
					obj->setIsDisappearByWeapn(true);
			...
			if (dynamic_cast<Enemy*>(obj))
			{
				...
				//Nếu Enemy không bất tử thì mới xét va chạm
				if (!e->getIsImmortal()) {
					...
					if (CheckCollision(left, top, right, bottom) == true)
					{
						...
						e->setIsDisappearByWeapn(true);	//Enemy biến mất là do weapon
...
//Set vị trí của MS dựa vào vị trí của Simon
...
void MorningStar::UpgradeMorningStar()
{
	...
	case MS_LEVEL1: {
		SetState(MS_LEVEL2);
		CRandomManager::GetInstance()->UpdateRandomMS(); //Cập nhật tỉ lệ random item
		break;
	}
	case MS_LEVEL2: {
		SetState(MS_LEVEL3);
