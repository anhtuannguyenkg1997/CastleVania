...
void PlayScene::_ParseSection_OBJECTS(string line)
{
	...
	objectCreater->CreateObject(tokens, simon, morningStar, simonPositionStart, simonState, simonOrientation);
...
void PlayScene::Update(DWORD dt)
{
	...
	//Cập nhật cho cell cho các object game
	grid->Update(listObjectCells);

	//Load những stair hiện có (bỏ những stair nằm ngoài vùng viewport)
	...
	if (simon != NULL && simon->GetState() != DEAD) {
		...
		//Xử lí với những màn mà khởi đầu là simon đứng trên thang
		cheatStair->CheckForCheat(id, simon);
		
		//Xử lí hidden object trong màn đó
	...
	if (rosaryEffect->isRosaryEffect && rosaryEffect->IsRosary()) {
		...
		for (UINT i = 0; i < listObjects.size(); i++)
		{
			...
			//Kiểm tra xem object có phải là MovingStair hay không
...
//Quản lí trạng thái của màn chơi (Time, Boss fighting, end game)
void PlayScene::UpdateStatePlayScene(DWORD dt) {
	...
	if (simon->GetState() != DEAD && !PlayerManager::GetInstance()->isFinishGame) {
		...
		if (remainTime == 0) {
			...
			remainTime = 0;			//Reset lại thời gian còn lại
		}
	...
	if (!PlayerManager::GetInstance()->isFinishGame) {
		...
		if (simon->GetState() == DEAD && GetTickCount() - simon->timeDied > SIMON_DEAD_TIME) {
			...
			if (playerManager->GetLive() == 1) {
				...
				//Dừng phát tất cả các sound để phát lại màn 1
				SoundManager::GetInstance()->StopAll();

				//Load lại toàn bộ game từ màn đầu
				sceneManager->Load(MAIN_FILEPATH);
			}
			//Còn mạng =>reload lại màn chơi
			else {
				playerManager->ResetData();		//Reset dữ liệu cho 1 màn chơi (giá trị của Simon, Boss)
				stairManager->isTouchingBreakDoubleStair = false; //xét lại va chạm với double stair
				...
				SoundManager::GetInstance()->Play(id);	//Phát sound của màn chơi hiện tại
				Unload();						//Unload màn hiện tại
...
void PlayScene::Render()
{
	...
	//Nếu Simon đang đánh mà không phải là dùng subweapon thì mới vẽ MorningStar
	//Render Morningstar dựa vào animation của simon. 
	//Sprite hành động của simon thế nào thì sprite của morningstar render đến đó
	...
	//Render Effect khi tiêu diệt Object nào đó bằng vũ khí của Simon
...
void PlayScene::Unload()
{
	...
	//Sẵn sàng cheat stair cho màn 3 và 5 (nếu chuyển đến 2 màn này)
	...
	//Time còn lại (dành cho 2 màn của cùng 1 stage)
	//Lưu lại để qua màn sau ta dùng lại time này
	...
	//Vì mỗi màn sẽ cho active hidden object 1 lần
