...
bool PlaySceneKeyForwardAction::canAccessKeyStateAction(Simon* simon) {
	...
	if (simon->GetState() == JUMPING && simon->animation_set->at(JUMPING)->isFinishAnimation(SIMON_JUMPING_TIME) == false) {
		return false;
	}
...
bool PlaySceneKeyForwardAction::canAccessKeyDownAction(Simon* simon) {
	...
	if (simon->GetState() == WHOPPING_STANDING && simon->animation_set->at(JUMPING)->isFinishAnimation(SIMON_JUMPING_TIME) == false) return true;
...
void PlaySceneKeyForwardAction::NotPressKeyAction(Simon* simon) {
	...
	stairManager->isTouchingBreakDoubleStair = false;
