...
void StairManager::Update(Simon* simon)
{
	...
	for (UINT i = 0; i < listStair.size(); i++)
	{
		...
		if (listStair[i]->GetBreakPoint() == 0) continue;
		...
		if (simon->BroadPhaseTestAABB(simonLeft, simonTop, simonRight, simonBottom, stairLeft, stairTop, stairRight, stairBottom))
		{
			...
			if (stairCollided->GetState() == 0) //Bậc thang dẫn từ trái qua phải, tương ứng với animation_set 0 của stair
				stairOrientation = STAIR_ORI_LEFT_TO_RIGHT;
			else								//Bậc thang dẫn từ phải qua trái, tương ứng với animation_set 1 của stair
				stairOrientation = STAIR_ORI_RIGHT_TO_LEFT;


			if (stairCollided->GetBreakPoint() == 2 && !isTouchingBreakDoubleStair) {
