...
MoneyBagItem::MoneyBagItem(float x, float y) : Item(x, y) {

	//Lấy Animation Của Item
	CAnimationSets* animation_sets = CAnimationSets::GetInstance();
	animation_set = animation_sets->Get(ITEM_MONEYBAG);
...
void MoneyBagItem::Update(DWORD dt, vector<LPGAMEOBJECT> &listObjects, Simon* simon) {
	...
	if (isEffectToSimon) {
		...
		temp += ITEM_MONEYBAG_SCORE;
		playerManager->SetScore(temp);

		//Hiển thị số điểm lên màn hình game
		playerManager->idScoreShow = ITEM_MONEYBAG_SCORE;
...
void MoneyBagItem::GetBoundingBox(float& left, float& top, float& right, float& bottom) {
	...
	bottom = top + ITEM_MONEYBAG_HEIGHT-5;
}